...
from app.schemas.auth import MemberInvite as MI
...
r = c.post('/api/v1/user/enterprise/members/invite', headers=owner_auth,
...
# members list (owner)
r = c.get('/api/v1/user/enterprise/members', headers=owner_auth)
...
if editor_auth:
    r = c.post('/api/v1/user/kb/facts', headers=editor_auth, json={
    ...
    r = c.post('/api/v1/user/enterprise/members/invite', headers=editor_auth,
...
if viewer_auth:
    r = c.post('/api/v1/user/kb/facts', headers=viewer_auth, json={
...
r = c.get('/api/v1/user/content/scenarios', headers=owner_auth, params={'page_size': 5})
scs = r.json().get('data', {}).get('items', []) if r.status_code == 200 else []
if not scs:
    r = c.post('/api/v1/user/content/scenarios', headers=owner_auth, json={
...
if sid:
    c.post(f'/api/v1/user/content/scenarios/{sid}/generate-drafts', headers=owner_auth)
r = c.get('/api/v1/user/content/drafts', headers=owner_auth, params={'page_size': 20})
...
for d in drafts_all[:3]:
    c.post(f"/api/v1/user/content/drafts/{d['id']}/machine-review", headers=owner_auth)

r = c.get('/api/v1/user/content/drafts', headers=owner_auth, params={'page_size': 20})
drafts_all = r.json().get('data', {}).get('items', []) if r.status_code == 200 else []
eligible = [d for d in drafts_all if d.get('status') in ('reviewed', 'fact_verified')][:3]
if eligible:
    r = c.post('/api/v1/user/content/drafts/bulk-approve', headers=owner_auth,
    ...
    r = c.get('/api/v1/user/content/drafts', headers=owner_auth,
...
if len(approved) >= 2:
    d_semi = approved[0]
    r = c.post('/api/v1/user/publish', headers=owner_auth, json={
    ...
    if semi_tid:
        r = c.post(f'/api/v1/user/publish/{semi_tid}/semi', headers=owner_auth,
...
r = c.post('/api/v1/user/monitor/trigger', headers=owner_auth, json={'pool': 'probe'})
...
r = c.get('/api/v1/user/monitor/core', headers=owner_auth, params={'page_size': 20})
step('GET /monitor/core 样本列表', r.status_code == 200,
     f'total={r.json().get("data",{}).get("total","?")}')
r = c.get('/api/v1/user/monitor/trend', headers=owner_auth, params={'pool': 'core', 'days': 7})
step('GET /monitor/trend 7d 趋势', r.status_code == 200,
     f'points={len(r.json().get("data",[])) if r.status_code == 200 else "?"}')
r = c.get('/api/v1/user/outcomes/dashboard', headers=owner_auth, params={'period': 'week'})
...
r = c.get('/api/v1/user/outcomes/ai-kpi', headers=owner_auth)
...
r = c.get('/api/v1/user/outcomes/traffic', headers=owner_auth)
...
r = c.post('/api/v1/user/outcomes/iterations/trigger', headers=owner_auth, json={'reason': 'weekly_review'})
