...
from geo_core.models import Scenario
...
class ScenarioService:
    ...
    def _load(it: Optional[Scenario]) -> Optional[Scenario]:
        if not it:
            return it
        target_engines_val = None
        fact_refs_val = None
        persona_ref_val = None
        competitor_ref_val = None
        try:
            target_engines_val = parse_list_from_str(getattr(it, "target_engines", "[]") or "[]")
        except Exception:
            target_engines_val = []
        try:
            ...
            fact_refs_val = []
        try:
            persona_ref_val = _jloads(getattr(it, "persona_ref", "{}") or "{}", {})
        except Exception:
            persona_ref_val = {}
        try:
            competitor_ref_val = _jloads(getattr(it, "competitor_ref", "{}") or "{}", {})
        except Exception:
            competitor_ref_val = {}
    ...
    def _save_dict(data: ScenarioCreate | ScenarioUpdate, payload: dict):
        if isinstance(data, ScenarioCreate) or (
            hasattr(data, "target_engines") and data.target_engines is not None
        ):
            engines = getattr(data, "target_engines", None)
            if engines is not None:
                payload["target_engines"] = to_list_str(engines)
    ...
    def create(db: Session, enterprise_id: int, data: ScenarioCreate) -> Scenario:
        ...
        payload["target_engines"] = to_list_str(data.target_engines)
        payload["fact_refs"] = to_list_str(data.fact_refs)
        payload["persona_ref"] = _jdumps(data.persona_ref or {})
        payload["competitor_ref"] = _jdumps(data.competitor_ref or {})
    ...
    def update(
        ...
        if "target_engines" in upd and upd["target_engines"] is not None:
            upd["target_engines"] = to_list_str(upd["target_engines"])
        ...
        if "persona_ref" in upd and upd["persona_ref"] is not None:
            upd["persona_ref"] = _jdumps(upd["persona_ref"])
        if "competitor_ref" in upd and upd["competitor_ref"] is not None:
            upd["competitor_ref"] = _jdumps(upd["competitor_ref"])
