...
from geo_core.models import KBFact, KBFaq, KBSignal, KBExternal, Enterprise
from geo_core.schemas.kb import (
    KBFactCreate,
    KBFactUpdate,
    KBFactResponse,
    KBFactListParams,
...
class _BaseKBService:
    ...
    def _create(
        ...
        if hasattr(data, "tags"):
            payload["tags"] = to_list_str(data.tags or [])
        if hasattr(data, "fact_refs"):
            payload["fact_refs"] = to_list_str(data.fact_refs or [])
    ...
    def _update(
        ...
        if "tags" in update_dict and update_dict["tags"] is not None:
            update_dict["tags"] = to_list_str(update_dict["tags"])
        if "fact_refs" in update_dict and update_dict["fact_refs"] is not None:
            update_dict["fact_refs"] = to_list_str(update_dict["fact_refs"])
    ...
    def _post_load(cls, item):
        ...
        if hasattr(item, "tags"):
            try:
                tags_val = parse_list_from_str(getattr(item, "tags", "[]") or "[]")
            except Exception:
                tags_val = []
        if hasattr(item, "fact_refs"):
            try:
                fact_refs_val = parse_list_from_str(getattr(item, "fact_refs", "[]") or "[]")
            except Exception:
                fact_refs_val = []
