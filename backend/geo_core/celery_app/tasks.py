...
from geo_core.celery_app.app import celery_app
from geo_core.core.logging_config import get_logger

logger = get_logger(__name__)


@celery_app.task(name="geo_core.celery_app.tasks.diagnosis_run", bind=True, acks_late=True)
def diagnosis_run(self, enterprise_id: int, scenario_id: int, **kwargs) -> Dict[str, Any]:
    ...
    try:
        from geo_core.services.diagnosis_service import DiagnosisService
        from geo_core.core.db import SessionLocal

        db = SessionLocal()
        try:
            result = DiagnosisService.run_diagnosis(db, enterprise_id, scenario_id)
...
@celery_app.task(name="geo_core.celery_app.tasks.content_generate", bind=True, acks_late=True)
...
@celery_app.task(name="geo_core.celery_app.tasks.monitor_sample", bind=True, acks_late=True)
