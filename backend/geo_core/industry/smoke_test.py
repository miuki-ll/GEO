...
print("=" * 60)
resp = client.get("/api/v1/kb/summary", headers=auth)
...
resp = client.post("/api/v1/kb/facts", headers=auth, json=new_fact)
...
print("=" * 60)
resp = client.get("/api/v1/diagnosis/pain", headers=auth)
mark("GET /diagnosis/pain", resp.status_code == 200, f"n={len(resp.json().get('data', []))}")
resp = client.get("/api/v1/diagnosis/persona", headers=auth)
mark("GET /diagnosis/persona", resp.status_code == 200, f"age_range={resp.json().get('age_range')}")
resp = client.get("/api/v1/diagnosis/competitor", headers=auth)
...
resp = client.post("/api/v1/strategy-pack/confirm", headers=auth)
...
print("=" * 60)
resp = client.get("/api/v1/content/scenarios", headers=auth)
...
if scenario_ids:
    sid = scenario_ids[0]
    resp = client.post(f"/api/v1/content/scenarios/{sid}/generate-drafts", headers=auth)
...
for did in draft_ids[:3]:
    resp = client.post(f"/api/v1/content/drafts/{did}/machine-review", headers=auth)
...
print("=" * 60)
resp = client.get("/api/v1/content/drafts", headers=auth, params={"page_size": 100})
all_drafts = resp.json().get("data", {}).get("items", [])
approved_ids = []
if draft_ids:
    bulk_ids = draft_ids[:3]
    resp = client.post("/api/v1/content/drafts/bulk-approve", headers=auth, json={"ids": bulk_ids, "note": "smoke bulk"})
    ok = resp.status_code == 200
...
published_task = None
if approved_ids:
    resp = client.post("/api/v1/publish", headers=auth, json={
    ...
    if resp.status_code in (200, 201):
        tid = resp.json()["id"]
        resp = client.post(f"/api/v1/publish/{tid}/auto", headers=auth)
...
print("=" * 60)
resp = client.post("/api/v1/monitor/trigger", headers=auth, json={"pool": "core", "scenario_ids": scenario_ids[:3]})
...
resp = client.post("/api/v1/monitor/trigger", headers=auth, json={"pool": "probe", "scenario_ids": []})
mark("POST /monitor/trigger probe", resp.status_code == 200, f"created={resp.json().get('data', {}).get('created', 0)}")

resp = client.get("/api/v1/monitor/core", headers=auth, params={"page_size": 10})
mark("GET /monitor/core list", resp.status_code == 200,
     f"total_samples={resp.json().get('data', {}).get('total', 0)}")
resp = client.get("/api/v1/monitor/trend", headers=auth, params={"pool": "core", "days": 7})
...
print("=" * 60)
resp = client.get("/api/v1/outcomes/dashboard", headers=auth, params={"period": "week"})
ok = resp.status_code == 200
...
resp = client.get("/api/v1/outcomes/ai-kpi", headers=auth)
mark("GET /outcomes/ai-kpi", resp.status_code == 200, "")
resp = client.get("/api/v1/outcomes/traffic", headers=auth)
...
resp = client.post("/api/v1/outcomes/iterations/trigger", headers=auth,
...
print("=" * 60)
resp = client.post("/api/v1/content/scenarios", headers=viewer_auth, json={
