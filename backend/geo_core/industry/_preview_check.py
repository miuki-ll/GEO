...
kb = GET('/api/v1/kb/summary')['data']
print(f'KB summary: facts={kb["facts"]} signals={kb["signals"]} verified={kb["verified_facts"]}')

sc = GET('/api/v1/content/scenarios?page_size=10')['data']
print(f'Scenarios: total={sc["total"]} items={len(sc["items"])}')

dr = GET('/api/v1/content/drafts?page_size=10')['data']
print(f'Drafts: total={dr["total"]}')

pub = GET('/api/v1/publish?page_size=10')['data']
print(f'Publish tasks: total={pub["total"]}')

mon = GET('/api/v1/monitor/core?page_size=10')['data']
print(f'Monitor core: total={mon["total"]}')

trend = GET('/api/v1/monitor/trend?pool=core&days=7')['data']
print(f'Trend 7d: points={len(trend)}')

dash = GET('/api/v1/outcomes/dashboard?period=week')['data']
