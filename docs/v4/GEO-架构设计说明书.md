# GEO 智能品牌平台 — 架构设计说明书 v4

> **版本**：V4.1 | **日期**：2026-07-04 | **关联**：PRD-v4.md · GEO-需求规格说明书.md · `二期.md`

---

## 0. 第一性原理（项目锚点）

> 内容价值 ≈ 信息增量 × 品牌信任度。架构服务于 **KB → scenario 内容 → 发布 → 可见度+转化监测 → 迭代草案**，而非批量发文。详见 PRD-v4 §0、需求规格 §3 转化闭环。
...
| Field 约束 | selected_scenario_ids≤5、字符串长度、enum channel/skill |
...
**onboarding/run：** 整体跑 `onboarding_graph`，节点 checkpoint → SSE 进度（**分步叙事+ETA**）；**job_id** 为 jobs 表与 LangGraph checkpoint **唯一关联键**（NFR-09）。
...
> PRD-v4 §2。后端 job 管道不变；前端按舱聚合；**2 次用户闸门**：方案包确认 + 草稿审。
...
KB_READY
  ...
  → AWAIT_STRATEGY_PACK
  → GAP_ANALYZE                # gap_analyze_graph · ReAct
  → PLAN(content_units per selected_scenarios, engine_plan, source_weight_plan, monitor_plan)
  → REVIEW_COMPLIANCE + REVIEW_FACT
...
#
# ⚠️ MVP Faiss 仅用于 pain/keyword 聚类；索引文件 **OSS 备份**；tenant 删除 **级联清理**（NFR-08）。
```
...
`source_weight_builder.py`：读 `source_diagnoses.payload.platform_stats` → 渠道映射表（行业包 `channels.json`）。**scenario_slots 总和 = len(selected_scenarios)**；UI 文案「本渠道承接几个 scenario」。
...
class XhsAdapter:
    async def publish(...):
        # MVP：**默认 SEMI**；AUTO 为可选尝试
        try: auto_post(...)  # P1 稳定后升默认
        except: return PublishResult(fallback="semi", export_bundle=...)  # title+body+tags+cover_hint+steps
```

凭证：`tenant_channel_credentials` 表，AES 加密；OAuth 优先。

**Gateway：** 按 tenant **rate limit**（FR-RB-03）；异常 LLM 调用熔断（完整预算熔断见二期 II-15）。
...
class MonitorScheduler:
    def run_weekly(tenant_id):
        core = profile.core_prompts   # MVP: KeywordLibrary.confirmed；V1.1: scenario-first（需求规格 §9.1）
...
| content_assets | fact_refs, machine_review, status, **approval_log JSONB** |
| publish_tasks | channel, mode, status, **content_asset_id**, published_at, fallback_semi |
...
**归因（MVP 骨架 · V1.1 补 ConsultLog）：** `publish_tasks.content_asset_id` + 7 天窗口 + `hosted_page_events`；V1.1 加 `consult_logs` 手工关联。详见需求规格 §3.2。
